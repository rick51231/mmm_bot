...
def bonus(bot, person, bot_setting):
    ...
    bot.send_photo(chat_id, img, caption=f'"Где взять деньги, если их нет!"', reply_markup=inline_keyboard)
...
def set_bot_logic(bot):
    ...
    def start(message):
        ...
        person, person_create = Person.objects.get_or_create(telegram_id=message.from_user.id, bot=bot_setting)
    ...
    def step_2(call):
        bot_setting, _ = Settings.objects.get_or_create(bot_id=bot.token)
        Person.update_status(call.from_user.id, bot_setting, 2)
        chat_id = call.message.chat.id
        text = f'*Шаг 2*'
        bot.send_message(chat_id, text, parse_mode='Markdown')
    ...
    def repeat_all_messages(message):
        bot_setting, _ = Settings.objects.get_or_create(bot_id=bot.token)
        person, _ = Person.objects.get_or_create(telegram_id=message.from_user.id, bot=bot_setting)
